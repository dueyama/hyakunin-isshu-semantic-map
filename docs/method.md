...
このファイルは公開リポジトリでは列構成だけを示すschema placeholderであり、解析に用いた百人秀歌本文は含みません。
...
- 第1章の主モデルは OpenAI `text-embedding-3-small`、入力は `original_kana`。
- 第2章・第3章の10×10検査は、百人秀歌側と小倉側をそろえるため、同モデルの作業用 `original` 条件を使う。
- 解析用本文と埋め込みベクトルは非公開とし、公開ページには集計値・図・方法だけを出す。

## 可視化

現行版では PCA を作成しています。UMAPとt-SNEは未実行で、将来の感度確認候補です。
...
- 百人秀歌101首は、1首ずつ枠外に置く101通りを試し、残る100首を10×10へ置く。
- 小倉百人一首は100首をそのまま置く。
- 通常行・列、つづら折り、斜めつづら折り、螺旋など、事前に定義した置き方を比べる。
- 上下左右の180組と、斜めを含む8近傍の342組を分けて集計する。
- ランダム基準は、同じ100首を固定した10×10格子へ無作為に置き直し、同じ180組または342組を測る操作である。独立な任意ペアを同数抽出する方法は使わない。
- 10,000回、seed `20260710`。外す歌、置き方、近傍定義、差し替え割当を複数試すため、最良値には選択バイアスがある。
...
## 追加比較: Jevによる季節判定（2026-09-20）

既存の埋め込み空間に、本文だけを入力としたJevの季節判定を重ねる。主な季節と季節への言及を分け、既存タグ・同季節近傍・共通歌の応答差を比較する。季節の首数を保つ歌単位のラベル置換は、従来の歌順置換や10×10配置置換とは別の帰無基準である。詳細な対象選択、ハッシュ照合、確信度の扱い、再現手順は [季節比較の方法](jev_season_method.md)、解釈は [補章](paper/seasons/) を参照。
